...
UWebSocketNetworkNetDriver::UWebSocketNetworkNetDriver(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

bool UWebSocketNetworkNetDriver::IsAvailable() const
{
	// Websocket driver always valid for now
	return true;
}

ISocketSubsystem* UWebSocketNetworkNetDriver::GetSocketSubsystem()
{
	return ISocketSubsystem::Get();
}

bool UWebSocketNetworkNetDriver::InitBase(bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error)
{
...
bool UWebSocketNetworkNetDriver::InitConnect(FNetworkNotify* InNotify, const FURL& ConnectURL, FString& Error)
{
	...
	ServerConnection = NewObject<UWebSocketNetworkConnection>(NetConnectionClass);
	...
	UWebSocketNetworkConnection* Connection = (UWebSocketNetworkConnection*)ServerConnection;
	Connection->SetWebSocket(WebSocket);

	FWebsocketPacketRecievedCallBack CallBack;
	CallBack.BindUObject(Connection, &UWebSocketNetworkConnection::ReceivedRawPacket);
	WebSocket->SetRecieveCallBack(CallBack);

	FWebsocketInfoCallBack  ConnectedCallBack;
	ConnectedCallBack.BindUObject(this, &UWebSocketNetworkNetDriver::OnWebSocketServerConnected);
...
bool UWebSocketNetworkNetDriver::InitListen(FNetworkNotify* InNotify, FURL& LocalURL, bool bReuseAddressAndPort, FString& Error)
{
	...
	CallBack.BindUObject(this, &UWebSocketNetworkNetDriver::OnWebSocketClientConnected);
...
void UWebSocketNetworkNetDriver::TickDispatch(float DeltaTime)
{
...
void UWebSocketNetworkNetDriver::LowLevelSend(FString Address, void* Data, int32 CountBits)
{
	...
	if (bValidAddress)
	{
		...
		if (CountBits > 0)
		{
			...
			{
				UWebSocketNetworkConnection* Connection = (UWebSocketNetworkConnection*)ClientConnections[i];
...
void UWebSocketNetworkNetDriver::ProcessRemoteFunction(class AActor* Actor, UFunction* Function, void* Parameters, FOutParmRec* OutParms, FFrame* Stack, class UObject* SubObject)
{
...
FString UWebSocketNetworkNetDriver::LowLevelGetNetworkNumber()
{
	return WebSocketServer->Info();
}

void UWebSocketNetworkNetDriver::LowLevelDestroy()
{
...
bool UWebSocketNetworkNetDriver::HandleSocketsCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld)
{
	Ar.Logf(TEXT(""));
...
bool UWebSocketNetworkNetDriver::Exec(UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
...
UWebSocketNetworkConnection* UWebSocketNetworkNetDriver::GetServerConnection()
{
	return (UWebSocketNetworkConnection*)ServerConnection;
}

void UWebSocketNetworkNetDriver::OnWebSocketClientConnected(FWebSocket* ClientWebSocket)
{
	...
	if (bAcceptingConnection)
	{
		...
		UWebSocketNetworkConnection* Connection = NewObject<UWebSocketNetworkConnection>(NetConnectionClass);
		check(Connection);
...
bool UWebSocketNetworkNetDriver::IsNetResourceValid(void)
{
...
void UWebSocketNetworkNetDriver::OnWebSocketServerConnected()
{
	check(GetServerConnection());
